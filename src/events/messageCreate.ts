...
export default event(
    ...
    async ({ log, msgHist, ollama, client }, message) => {
        const clientId = client.user!!.id
        const cleanedMessage = clean(message.content, clientId)
        log(
            `Message \"${cleanedMessage}\" from ${message.author.tag} in channel/thread ${message.channelId}.`
        )

        // Do not respond if bot talks in the chat
        if (message.author.username === message.client.user.username) return

        // Prevent everyone mention
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                getServerConfig(`${message.guildId}-config.json`, (config) => {
                    ...
                    if (config === undefined) {
                        ...
                        openConfig(
                            `${message.guildId}-config.json`,
                            'toggle-chat',
                            true
                        )
                        reject(
                            new Error(
                                'No Server Preferences is set up.\n\nCreating default server preferences file...\nPlease try chatting again.'
                            )
                        )
                        return
                    }
            ...
            const userConfig: UserConfig = await new Promise(
                (resolve, reject) => {
                    getUserConfig(
                        ...
                        (config) => {
                            ...
                            if (
                                typeof config.options['modify-capacity'] !==
                                'number'
                            )
                                log(
                                    `Capacity is undefined, using default capacity of ${msgHist.capacity}.`
                                )
                            else if (
                                config.options['modify-capacity'] ===
                                msgHist.capacity
                            )
                                log(
                                    `Capacity matches config as ${msgHist.capacity}, no changes made.`
                                )
                            else {
                                log(
                                    `New Capacity found. Setting Context Capacity to ${config.options['modify-capacity']}.`
                                )
                                msgHist.capacity =
                                    config.options['modify-capacity']
                            }
                            ...
                            // set stream state
                            shouldStream =
                                (config.options['message-stream'] as boolean) ||
                                false

                            if (
                                typeof config.options['switch-model'] !==
                                'string'
                            )
                                config.options['switch-model'] =
                                    Keys.defaultModel
            ...
            let chatMessages: UserMessage[] = await new Promise((resolve) => {
                ...
                getChannelInfo(
                    `${message.channelId}-${message.author.username}.json`,
                    (channelInfo) => {
                        if (channelInfo?.messages) resolve(channelInfo.messages)
                        else {
                            log(
                                `Channel/Thread ${message.channel}-${message.author.username} does not exist. File will be created shortly...`
                            )
                            resolve([])
                        }
                    }
                    ...
                    getChannelInfo(
                        ...
                        (channelInfo) => {
                            if (channelInfo?.messages)
                                ...
                                reject(
                                    new Error(
                                        `Failed to find ${message.author.username}'s history. Try chatting again.`
                                    )
                                )
                            }
                        }
            ...
            const messageAttachment: string[] = await getAttachmentData(
